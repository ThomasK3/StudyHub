...
```javascript
{
  ...
  code: string,            // Kód předmětu z InSIS, např. "4IT115"
  ...
  lecturer: string,        // Jméno přednášejícího
  
  // Složky hodnocení
  components: [
    {
      name: string,        // Název, např. "Průběžný test"
  ...
  insisUrl: string,        // Odkaz na InSIS stránku předmětu (volitelné)
...
Všechna uživatelská data se ukládají v `localStorage`:

```javascript
// Klíče
"studyhub_courses"    // JSON pole předmětů
"studyhub_semester"   // Aktuální semestr
"studyhub_settings"   // Uživatelské nastavení
```

### Export / Import

- **Export:** Stáhnout JSON soubor se všemi daty
- **Import:** Nahrát JSON soubor (validace + merge/overwrite)
- Formát kompatibilní mezi verzemi (migrace)

### Budoucí rozšíření

- Supabase / Firebase pro cloudovou synchronizaci (volitelné)
- Sdílení mezi studenty (export specifického předmětu)
...
## 11. Scope a fáze vývoje

### Fáze 1 — MVP (aktuální)

- [x] Designový základ (FIS branding)
- [ ] Vanilla JS přepis (z React prototypu)
- [ ] Formulář pro zadávání předmětů (se select listem FIS předmětů)
- [ ] Dashboard s kartami předmětů
- [ ] Detail předmětu
- [ ] Kalendář s týdny výuky
- [ ] localStorage persistence
- [ ] Export/import JSON

### Fáze 2 — AI funkce

- [ ] AI parsování sylabů (předvyplnění formuláře)
- [ ] AI plánovač zkouškového
- [ ] Lepší onboarding flow

### Fáze 3 — Rozšíření

- [ ] Podpora dalších fakult (FFÚ, FPH, NF, FMV, FŘ)
- [ ] Katalogy předmětů pro každou fakultu
- [ ] Sdílení dat mezi studenty
- [ ] PWA (offline přístup, push notifikace)
